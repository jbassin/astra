...
export function Keys() {
  ...
  async function generate() {
    const name = await promptText({
      title: "New API key",
      label: "Name this key (e.g. 'Stream Deck')",
      defaultValue: "Stream Deck",
    });
    if (name === null) return;
    try {
  ...
  return (
    <section className="keys card">
      <button
        type="button"
        className="keys__toggle btn btn--ghost"
        onClick={() => setOpen((o) => !o)}
      >
      ...
      {open && (
        <div className="keys__body">
          ...
          <p className="muted">
            Paste a key into the orator-controller (Stream Deck) plugin settings to drive playback.
          </p>
        </div>
      )}
    </section>
