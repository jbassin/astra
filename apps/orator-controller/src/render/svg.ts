...
const BG = "#16181d";
const FG = "#e6e8ec";
const ACCENT = "#4aa6a0"; // teal, matches the gothic/orator family
...
export function wrapLines(text: string, maxChars: number, maxLines: number): string[] {
  ...
  if (consumed < words.length && l !== undefined) {
...
/** A fixed tag button. Dim when the named tag didn't resolve to a orator tag. */
export function navTagSvg(
  label: string,
  color: string | null,
  active: boolean,
  resolved: boolean,
