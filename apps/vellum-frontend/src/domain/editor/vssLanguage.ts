/**
 * VSS editor grammar. An `@lezer/markdown` extension that parses the VSS
 * structural surface into real syntax-tree nodes. Mirroring the compiler's
 * philosophy — braces for structure, markdown for content — it *extends* the
 * editor's markdown grammar with block parsers for the structural lines only;
 * body content stays ordinary markdown (so emphasis, headings, sigils, etc.
 * inside a `{ … }` body keep their normal highlighting). Ported verbatim from
 * faerrin pkg/vellum (import repointed at @astra/vellum-lang; highlight colours
 * remapped to gothic's `--color-*` tokens).
 *
 ...
 *
 * Faithfulness to the compiler (vellum-lang `vss.ts`):
...
import { DOCUMENT_KINDS } from "@astra/vellum-lang";
...
export const vssHighlighting: Extension = syntaxHighlighting(
  HighlightStyle.define([
    { tag: kindTag, color: "var(--color-accent)", fontWeight: "600" },
    { tag: titleTag, color: "var(--color-accent-amber)" },
    { tag: attrKeyTag, color: "var(--color-ink-dim)" },
    { tag: braceTag, color: "var(--color-accent)", fontWeight: "600" },
  ]),
);
...
function parseVssOpener(cx: BlockContext, line: Line): boolean {
  ...
  if (!m) return false;
  const children = [{ type: "VSSKind", from, to: from + 1 + m[1]!.length }];
  if (m[2]) {
    const open = text.indexOf('"', 1 + m[1]!.length);
    children.push({ type: "VSSTitle", from: from + open, to: from + m[0].length });
  }
...
function parseVssAttr(cx: BlockContext, line: Line): boolean {
  ...
  if (!m) return false;
  const from = cx.lineStart + line.pos;
  return claimLine(cx, line, "VSSAttr", [{ type: "VSSAttrKey", from, to: from + m[1]!.length }]);
}
