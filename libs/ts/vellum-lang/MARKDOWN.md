# Vellum markdown dialect (full-vellum)

A precise reference for the markdown flavor `@astra/vellum-lang` parses. This is the
**full-vellum** dialect — the single authored-content format for the whole akasha setting
wiki, not just PF2e statblocks. It describes **exactly what the parser does** (`src/parse.ts`
+ the construct modules); the React **renderer lives in gothic** (sub-plan 0003), not here.

> **astra split:** `@astra/vellum-lang` (this package) is the **TS reference parser** →
> `VellumDocument` AST; `astra-vellum-lang` (Python) is a **metadata-only** extractor
> (frontmatter + crossref set). The shared conformance corpus (`fixtures/vellum/`) is the
> spec; the `.meta.json` parity gate proves the two agree.
...
parchment) is chosen in the viewer/export request — **there is no in-document syntax
for it**. (Leading YAML **frontmatter** *is* parsed for metadata — §11.1 — but does not
set the theme; that stays a viewer setting.) The *same* source
...
## 11. Full-vellum constructs (akasha wiki)

Four constructs grow the dialect from "PF2e document forge" to **the single content format
for the whole setting wiki**. They are parsed into the AST here; gothic (0003) renders them.
All are pure/total (never throw). The Python extractor parses **§11.1 frontmatter** + the
**§11.2 crossref set** only (the metadata akasha-backend needs); §11.3/§11.4 are TS-only AST.

### 11.1 Frontmatter

A leading `---` … `---` YAML block (Obsidian-compatible) is split off and validated into a
typed struct on `VellumDocument.frontmatter`:

```
---
title: Belvedere
tags: [Watcher, Religious]
aliases: The Enclave        # a scalar or a list — both normalize to string[]
---
Body starts here.
```

- Known keys: `title?`, `tags: string[]`, `aliases: string[]`, `img?`. Every **other** key is
  preserved verbatim in `extra` (akasha may add `kind`/`folder`/`slug`).
- **Total:** absent / blank / malformed YAML → empty frontmatter, never an error.

### 11.2 Cross-references — `[[target#heading|alias]]`

Obsidian wikilinks are a first-class **inline** node (`crossref`). Keeping the syntax means the
wiki migrates with ~zero link rewriting.
...
- Forms: `[[target]]`, `[[target|alias]]`, `[[target#heading]]`, `[[target#heading|alias]]`.
- vellum-lang **parses only** — it does **not** resolve a target to a URL or entity. Resolution
  + the backlink graph are **akasha-backend's** job (0007), which has the page index + ontology.
- `[[…]]` inside `` `inline code` `` and fenced code blocks (``` ``` ``` / `~~~`) is **not** a
  crossref. *(Metadata-scan residuals: the lightweight Python scan does not exclude `[[…]]` in
  4-space-indented code or multi-backtick spans, and `[[A\|B]]` / `[[Tar*get*]]` with escapes or
  markup in the target diverge from the TS parser. A `[[wikilink]]` inside code or with markup in
  the target is effectively nonexistent in the corpus, so these stay documented edges, not bugs.)*

### 11.3 Field-lists — `:::fields`

The deity/stat `**Field** :: value` convention becomes a structured, queryable block:
...
- Each line is one `Term :: value` item. `term` is flattened to text (so akasha can query a
  deity's "Domains"); `value` keeps its inline content — links, `[[crossrefs]]`, emphasis.
- The ` :: ` split is **scoped to inside `:::fields`** so it never collides with inline `:`
  directives. A line with no `::` becomes a term with an empty value (degrade, don't drop).

### 11.4 Timeline — `:::timeline`

Replaces the wiki's raw `<ul><li style=…>` HTML with a structured block:
...
- The body is a list; each item's optional leading `{marker}` (era/date) is pulled off and the
  rest becomes the entry's content. A non-list body → one marker-less entry (degrade, don't drop).

### Still not supported

- **Raw HTML stays inert** (the security rule carries over). `Timeline.md`'s `<ul><li>` is
  **converted** to `:::timeline` by the akasha converter (0007), never rendered as HTML.
- **Transcripts are NOT vellum** — the generated `Script/` pages are akasha-frontend rendering fed
  by linguist data, not vellum documents; there are no transcript directives.
