...
const { port } = loadSiteConfig(ROOT).mouthpieceFrontend;

// SSR — no `prerender` block (Decision I): mouthpiece-frontend runs as a TanStack
// Start server, a Compose service behind Caddy, not prerendered to static `dist/`.
export default defineConfig({
  server: { port, host: true },
  resolve: {
    alias: { "@": path.resolve(ROOT, "./src") },
  },
  // contentWatchPlugin runs build-content (→ src/generated/*.ts) at buildStart +
  // re-runs on content edits in dev, so the episode-manifest read never reaches the
  // client bundle. @tailwindcss/vite compiles gothic's theme.css (`@theme` tokens +
