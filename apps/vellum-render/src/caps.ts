 ...
 * input is bounded here, before anything reaches the browser. Ported verbatim
 * from faerrin pkg/vellum (ThemeMode now from @astra/vellum-lang).
 */
...
export function validateRenderRequest(body: unknown): Validation {
  ...
  if (record.scale !== undefined) {
    ...
    if (record.scale < RENDER_LIMITS.minScale || record.scale > RENDER_LIMITS.maxScale) {
      return {
        ok: false,
        status: 422,
