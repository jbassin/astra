...
export function orderEdgesIntoPath(edges: ReadonlyArray<EdgeSegment>): EdgeSegment[] {
  if (edges.length === 0) return [];

  const pointKey = (x: number, y: number): string => `${x.toFixed(6)},${y.toFixed(6)}`;
...
// (|dq| + |dq+dr| + |dr|) / 2.
export function axialDistance(a: readonly [number, number], b: readonly [number, number]): number {
  const dq = a[0] - b[0];
  const dr = a[1] - b[1];
