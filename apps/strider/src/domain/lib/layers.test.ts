...
describe("foldRegions", () => {
  ...
  it("update only changes provided fields, leaving others intact", () => {
    const result = foldRegions([
      ...
      layer("two", "2026-01-02T00:00:00Z", [{ op: "update", slug: "hq", name: "New HQ" }]),
    ]);
  ...
  it("throws when updating a slug that does not exist", () => {
    expect(() =>
      foldRegions([
        layer("one", "2026-01-01T00:00:00Z", [{ op: "update", slug: "ghost", name: "X" }]),
      ]),
  ...
  it("throws when removing a slug that does not exist", () => {
    expect(() =>
      foldRegions([layer("one", "2026-01-01T00:00:00Z", [{ op: "remove", slug: "ghost" }])]),
    ).toThrow(/does not exist/);
  });
...
describe("foldSkein", () => {
  ...
  it("throws when skein-connect references self", () => {
    expect(() =>
      foldSkein([
        layer("one", "2026-01-01T00:00:00Z", [{ op: "skein-connect", from: "a", to: "a" }]),
      ]),
    ).toThrow(/itself/);
  });
  ...
  it("throws when skein-disconnect targets a pair that isn't connected", () => {
    expect(() =>
      foldSkein([
        layer("one", "2026-01-01T00:00:00Z", [{ op: "skein-disconnect", from: "a", to: "b" }]),
      ]),
...
describe("foldFactionOverrides", () => {
  ...
  it("last claim wins for the same hex across layers", () => {
    const result = foldFactionOverrides([
      layer("one", "2026-01-01T00:00:00Z", [{ op: "claim", faction: "a", hexes: [[0, 0]] }]),
      layer("two", "2026-01-02T00:00:00Z", [{ op: "claim", faction: "b", hexes: [[0, 0]] }]),
    ]);
  ...
  it("preserves explicit null as a distinct value from absent", () => {
    const result = foldFactionOverrides([
      layer("one", "2026-01-01T00:00:00Z", [{ op: "claim", faction: null, hexes: [[5, -2]] }]),
    ]);
