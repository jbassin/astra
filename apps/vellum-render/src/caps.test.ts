...
describe("validateRenderRequest", () => {
  ...
  test("rejects out-of-range scale (SEC-4 / R-16 cap)", () => {
    ...
    expect(validateRenderRequest({ source: "x", scale: RENDER_LIMITS.maxScale + 1 }).ok).toBe(
      false,
    );
  ...
  test("rejects unknown mode", () => {
    expect(validateRenderRequest({ source: "x", mode: "purple" }).ok).toBe(false);
    expect(validateRenderRequest({ source: "x", mode: "diegetic" }).ok).toBe(true);
  });
});
