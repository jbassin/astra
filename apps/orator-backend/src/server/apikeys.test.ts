...
describe("api keys", () => {
  test("generates raw + matching hash + prefix", () => {
    const k = generateKey();
    expect(k.raw.startsWith("orator_")).toBe(true);
  ...
  test("verifyKey accepts the right key, rejects others", () => {
    ...
    expect(verifyKey("orator_wrong", k.hash)).toBe(false);
  });
  ...
  test("extractApiKey reads Bearer and X-Orator-Key", () => {
    expect(extractApiKey(new Headers({ authorization: "Bearer abc" }))).toBe("abc");
    expect(extractApiKey(new Headers({ "x-orator-key": "xyz" }))).toBe("xyz");
