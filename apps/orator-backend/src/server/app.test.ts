...
function testConfig(overrides: Partial<AppConfig> = {}): AppConfig {
  return {
    ...
    oauth: {
      clientId: "cid",
      clientSecret: "sec",
      redirectUri: "https://orator.test/auth/callback",
    },
...
function makeApp(cfg = testConfig(), deps = {}) {
  return createApp(cfg, new FakeStore(), deps);
}

function get(path: string, headers: Record<string, string> = {}) {
  return new Request(`https://orator.test${path}`, { headers });
}
...
describe("health + auth guard", () => {
  ...
  test("/api/v1/me returns uid for an allowlisted session", async () => {
    ...
    const res = await makeApp().handle(get("/api/v1/me", { cookie: `orator_session=${token}` }));
    expect(res.status).toBe(200);
...
describe("oauth login + callback", () => {
  ...
  test("callback exchanges code, sets session, redirects home", async () => {
    ...
    expect(res.headers.get("set-cookie")).toContain("orator_session=");
  });
  ...
  test("a bot-install redirect (guild_id/permissions) shows guidance, not an error", async () => {
    const res = await makeApp().handle(
      get("/auth/callback?code=c&guild_id=753&permissions=3145728"),
    );
    expect(res.status).toBe(200);
