/**
 * Popover hover-previews — React port of faerrin's Solid island (Quartz popover).
 * Progressive-enhancement: renders nothing, binds mouseenter/leave to every resolved
 * internal link on mount. On hover it fetches the target page, extracts its
 * `.popover-hint` (every page wraps its body in one), prefixes inner ids to avoid
 * dupes, and floats a card positioned with @floating-ui/dom.
 *
 * Attaches to `a[data-crossref]` (the slice-4 gothic crossref links) AND `a.internal`
 * (backlinks / tags / breadcrumbs / listings). Re-runs on route change so the new
 * page's links get bound and the old ones torn down (N5 — astra is a SPA; faerrin
 * was MPA and re-mounted per page).
 */
