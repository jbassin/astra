...
#: (24_000) so "previously on" never eats into the akasha grounding window.
CONTINUITY_BUDGET = 6_000

#: Beats shown for the most-recent prior episode (older ones get title + synopsis only).
_MAX_RECENT_BEATS = 3


def _episode_line(entry: EpisodeEntry, *, detailed: bool) -> str:
    """One prior-episode bullet: title + synopsis, plus cliffhanger + a few beats if detailed."""
    ...
    beats = [b for b in summary.key_beats if b.strip()][:_MAX_RECENT_BEATS]
    if beats:
...
def build_continuity_block(
    ...
    `prior` is oldest→newest; the MOST-RECENT episode gets its cliffhanger + a few beats,
    older ones a one-line title+synopsis. A best-effort `season` adds an arc-framing line.
    Episode detail is dropped least-recent-first once the block exceeds `budget` chars (the
    season line + the most-recent episode are always kept); a final hard cap is a backstop.
    """
    ...
    if prior:
        ...
        lines = [_episode_line(entry, detailed=(i == last)) for i, entry in enumerate(prior)]
