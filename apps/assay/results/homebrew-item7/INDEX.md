...
| connection | **Keep as-is (A)** — the deferred batch-0 conch/pearl point CLOSED: pearl stays stripped (Telepathic Bond r5 official = 8h multi-party planetary telepathy, no cost — a tax here has no balance job); priced fair vs Sending (durative vs one-shot); conch line stays pure flavor; converter-notes Sending errata (no subtle trait, 3A cast) recorded as context only | `d914cac` |
| ebb-and-flow | **Rank 3→4 + register cleanup (A+)** — his own checklist remedy taken (Everything-Spell: Haste-lite + Slow-lite AoE at the single-target anchors' rank); redundant Critical Success tier deleted (Slow idiom — unlisted crit success = unaffected); labeled Allied/Enemy Effect blocks flattened to normal spell prose (no official spell uses them); Will kept (temporal-perception reasoning stands); dice/effects untouched | `6ecd28a` |
| lockstep-fate | **Hold/discharge lifecycle tightened (A)** — the queued text bug fixed: vestigial re-hold sentence → 5e's actual no-replace constraint; H4 = hold two, choose on the single discharge, unused roll lost; H6 = discharge ends that creature's link only, remaining links end on their own discharge/sustain-stop/expiry; crit-fail nat-1 kept (the fun tier); mechanics + score unchanged (r2 ledger:utility) | `17ee54f` |
| fault-line (+hypercompression, raise-island) | **Divine restored family-wide + range 500→120 (A-extended)** — divine added to ALL THREE terrain-scale kosmoturgy holdouts (5e Cleric/Paladin lineage; 28/28 school spells now carry divine); Fault Line range 500→120 ft (stakeholder trim); prone-on-failure + 8d10 kept (his flagged, compensated trade); verdict −0.63→+1.12 HOT = the DOCUMENTED range-bucket lens artifact (blunt >1-rank lever, dice untouched, spell strictly weaker at table) | `4411ce7` |

**Fast lane (73 spells, 2026-07-23; PROCESS CHANGE stakeholder-directed: compact cards, stop only where staff read finds a real decision; policy-covered fixes applied + logged without stopping):**

| Spell | Decision | Commit |
|---|---|---|
| disrupt-movement · taboo | **Crit-success dedup (staff, Ebb-and-Flow convention)** — redundant "Critical Success = unaffected" tier deleted from both; taboo's curse text verified already rank-explicit (batch-0) | `c0b4a92` |
| chrysalis | **GM-fiat sentence deleted (staff)** — "outside of combat / no active threat" redundant with the 10-minute cast; converter self-flagged it non-standard | `c0b4a92` |
| lend-time | **Trigger line formatted (staff, planar-shield precedent)** — plain "Trigger:" prose → official bold form | `c0b4a92` |
| planar-pyre | **5e fade clause restored (staff)** — non-lethal spikes crumble at start of your next turn (5e's cleanup beat; slain-persistence clause already survived); −0.68 COLD stable | `59b791c` |
| sphere-of-preservation | **Queued Ruin-synergy applied (stakeholder, from the sphere-of-ruin card)** — reciprocal sentence added (counteract attempts −2 while both spheres sustained) + H10's redundant fortune-suppression restatement trimmed (base effect already) | `59b791c` |
| nightfall | **Rarity → uncommon (staff, precedent ×2)** — region-scale time-shift (10-mile H6), Farsight/Anomalous-Object plot-scale logic, converter's own lean | `c18d93e` |
| do-my-bidding | **Emanation range idiom (staff)** — range field emptied for the self-centered 60-ft emanation per official convention; ledger routing unchanged | `c18d93e` |
| flicker → **flutterstep** | **Renamed (stakeholder) + flat-check idiom (staff)** — name collided with the official rank-4 Flicker (same trait profile); granted ability → "Flutter Step"; the d20-threshold dodge → DC 11 flat check (DC 8 @H5), the established 5e-ism map; "abilities" plural verified correct (Step + Dodge); seededFrom pairing intact | see commit |
| fluid-form | **5e immunities restored (A)** — immunity to poison + the Stunned condition added back beside Paralyzed (the unacknowledged drop; liquid-body fiction); swim/Squeeze additions + Slowed-1 expulsion redesign kept; routing unchanged (buff 2–8) | see commit |
| incensed-bestial-rage | **Exhaustion exit-cost restored (A)** — "When Incensed Bestial Rage ends, you become Fatigued" (5e's level-of-exhaustion governor, exact idiom map); the conversion's sweeteners (temp HP, Athletics) kept; routing unchanged | see commit |
| lucky-ward | **Keep as-is (A)** — the dropped ally-save half is load-bearing for the rank: attacks-only symmetric aura prices correctly vs Bless+Bane (both r1) at r2; ally saves back would be Everything-Spell at the wrong rank; the crit-fail rider keeps the save axis as a spike | (log-only) |
| mental-balance + move-the-cosmic-wheel | **Cheap-ritual-props residue CLOSED (B)** — the two non-consumed Cost props resolved: Cosmic Wheel's 250-gp attuned music box → official Requirements line (prose "wind your music box" kept); Mental Balance's blindfold/scales dropped entirely (prose never referenced them). All other cost lines verified settled: ritual Costs ×3, Requirements ×3, consumed-with-gp costs = Everlight-precedent legal, each kept at its card | see commit |
| official-trait pass (13 spells) | **17 official traits added (stakeholder)** — attack ×4 (Ashen Pack, Darkseeker's Aura, Eldritch Horror, Kosmoturgist's Weapon) · aura ×5 (Arcane Interdiction, Righteous Pressure, Sphere of Preservation, Solar Fury, Thaumaturgic Obstruction) · spirit, force, fear+emotion ×2, sleep ×2, darkness (Nightfall), teleportation (Haunt); ~50 candidates pruned (negation guards, detect≠inflict, static emanations, Stunned-1 riders per Slow precedent, official move=caster-movement); zero score drift set-diff-proven | `187865d` |
| 500-ft range review (5 spells) | **3 keep / 2 fixed (stakeholder)** — Almonk's Retribution + Falling Star keep (Fireball r3 = 500 ft artillery class) · Illusory Illusion keep (Illusory Object r1 = 500 ft) · Gravity Anvil → 120 ft (single-target sniping has no official analog; Disintegrate register) · Sapping Lightning range emptied (prose says "from your fingertips" — self-origin line, the 500 was contradictory data); both flipped in-band→HOT (+2.30/+0.56) = the range-bucket artifact again, dice untouched | see commit |
| tradition passes (53 spells) | **Primal pass + mercuromancy reshape (stakeholder)** — chronomancy + planara gain primal school-wide (5e lineage: Druid 28/28, Ranger 25/25 — the Fault Line logic applied consistently; rituals' emptied lists untouched); mercuromancy → uniformly [divine, occult] ("primarily divine and occult, not arcane or primal") with ONE stakeholder exception — Gift of the Archmage stays arcane-only (identity); spread occult 118 · arcane 109 · primal 80 · divine 62; zero score drift set-diff-proven | `7af5e8b` + exception |
| ~61 keeps (batch 1, incl. almonk-s-arcane-drain) | **Kept as-is after staff read** — all flags policy-covered or stale: school-trait swaps (antillurgy-for-abjuration etc. = the 8-school policy), materials scrub (mental-balance's surviving blindfold/scales rides the queued cheap-ritual-props residue), schema-limit heighten representation, dropped 5e ritual flags on sub-1-hour casts (>1h rule settles), 1A→2A map, stale markdown-formatting flags (batch-0 already fixed left-hand-of-judgment/temporal-discharge/temporal-threshold/wall-of-time), propagating-blast damage flag stale (batch-0 structured it), jolt/revisit empty-field flags = official instantaneous idiom, bodydouble teleportation trait defensible (attacks originate at the projection), lyrr's-shell cast trim fine (non-combat utility) | (log-only) |
