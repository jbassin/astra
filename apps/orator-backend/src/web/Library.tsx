...
export function Library() {
  ...
  async function runBulkRename(ops: RenameOp[], emptyMsg: string) {
    const { preview } = await apiSend<{
      preview: { from: string; to: string; changed: boolean }[];
    }>("POST", "/api/v1/tracks/bulk-rename", { ids: selectedIds, ops, preview: true });
  ...
  async function stripPrefix() {
    const value = await promptText({
      title: "Strip prefix",
      label: "Remove this text from the START of every selected title:",
    });
    if (!value) return;
    await runBulkRename(
  ...
  async function stripSuffix() {
    const value = await promptText({
      title: "Strip suffix",
      label: "Remove this text from the END of every selected title:",
    });
    if (!value) return;
    await runBulkRename(
  ...
  async function bulkTag() {
    const raw = await promptText({
      title: "New tag",
      label: "Tag(s) to add to selected (comma-separated):",
  ...
  async function removeTag(removeTagId: number) {
    if (!selected.size) return;
    await withBusy(async () => {
      await apiSend("POST", "/api/v1/tracks/bulk-tag", {
        ids: selectedIds,
  ...
  async function play(trackIds: number[]) {
    ...
    try {
      ...
      toast.error(
        "Playback unavailable — is the Discord bot online and are you in a voice channel?",
      );
    }
  }
  ...
  async function upload(files: FileList | null) {
    ...
    await withBusy(async () => {
      try {
        ...
        if (!res.ok) {
          toast.error(
            `Upload failed: HTTP ${res.status} ${(await res.text().catch(() => "")).slice(0, 200)}`,
          );
          return;
        }
        ...
        if (out.errors?.length) {
          toast.error(
            `Skipped ${out.errors.length} file(s): ${out.errors.map((e) => `${e.name} (${e.error})`).join("; ")}`,
          );
        }
        await loadFacets();
        await loadTracks();
      ...
      <aside className="lib__facets">
        ...
        <ul className="lib__list">
          ...
          {collections.map((c) => (
            <li key={c.id} className="lib__collrow">
              <button
                type="button"
                className={collectionId === c.id ? "is-active" : ""}
                onClick={() => setCollectionId(c.id)}
              >
                {c.name}
              </button>
              <span className="lib__collacts">
                <button type="button" title="Rename" onClick={() => void renameCollection(c)}>
                  ✎
                </button>
                <button
                  type="button"
        ...
        <ul className="lib__list">
          <li>
            <button
              type="button"
              className={tagId === null ? "is-active" : ""}
          ...
          {tags.map((t) => (
            <li key={t.id} className="lib__tagrow">
              <button
                type="button"
                className={tagId === t.id ? "is-active" : ""}
        ...
        <div className="lib__selbar">
          ...
          <Menu
            ...
            items={[
              ...
              ...collections.map((c) => ({
                label: c.name,
                onSelect: () => void moveSelected(String(c.id)),
              })),
            ]}
          />
          <button
              ...
              <tbody key={sec.key}>
                <tr className="lib__secthead">
                  <td colSpan={3}>
                    <span
                      ...
                      style={{
                        background: sec.color ?? "transparent",
                        borderColor: sec.color ?? "var(--muted)",
                      }}
                    />
                ...
                {sec.tracks.map((t) => (
                  <tr
                    ...
                    style={rowStyle(sec.color, selected.has(t.id))}
                  >
                    <td>
                      <input
                        type="checkbox"
                    ...
                    <td>
                      ...
                      <button
                        type="button"
                        className="lib__title"
                        onClick={() => void renameOne(t)}
