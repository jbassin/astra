// Layer-writer sidecar for the /editor page. Run alongside `bun dev`:
//   bun run editor:server
// Binds 0.0.0.0:3001 so it is reachable inside the Compose network; this is an
// open write path by design — access control is enforced at the Caddy edge
// (Decision I deploy, slice 6), not here. Input is still hard-validated below
// (filename regex, path-traversal guard, size cap, no-overwrite).
...
console.log(`[editor-server] listening on http://${server.hostname}:${server.port}`);
