...
async function buildFactions(): Promise<Faction[]> {
  ...
  const factions = await Promise.all(files.map((f) => parseFaction(path.join(FACTIONS_DIR, f))));
...
async function main(): Promise<void> {
  ...
  const { allBorders: factionBorders, perFaction: territoryBorders } = computeAssignmentBorders(
