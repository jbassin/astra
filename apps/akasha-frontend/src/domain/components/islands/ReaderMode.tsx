/**
 * ReaderMode toggle — React port of faerrin's Solid island (Quartz readermode).
 * Toggles `<html reader-mode="on|off">` (CSS fades the sidebars) and dispatches
 * `readermodechange`. State is non-persistent (resets each load), so no head
 * pre-paint script is needed. onMount→useEffect, onCleanup→cleanup return (N5).
 */
...
export function ReaderMode() {
  ...
  return (
    <button ref={btn} className="readermode" type="button">
      <svg
        className="readerIcon"
        fill="currentColor"
        stroke="currentColor"
        strokeWidth="0.2"
        strokeLinecap="round"
