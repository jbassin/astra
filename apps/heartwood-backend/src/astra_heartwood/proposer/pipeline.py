...
def _draft_lint_assemble(
    ...
    *,
...
) -> tuple[str, list[VoiceWarning]] | None:
    """Draft → lint → bounded revise → assemble one page.

    Returns ``(vellum_text, residual_lints)``, or None when a rewrite is ``already-known`` (P3.15 —
    ...
    if prose_tells:  # one bounded revise (P3.6) — keep whichever draft is cleaner
        revised = revise_draft(body, prose_tells, client=client, model=model)
        revised_warnings = lint(revised)
        revised_tells = [w for w in revised_warnings if w.type in PROSE_TELL_TYPES]
        if len(revised_tells) < len(prose_tells):
            body, warnings = revised, revised_warnings

    proposal.page_type = page_type
    proposal.conflicts = draft.conflicts
    proposal.lints = warnings
    return assemble_vellum(body, existing_text=existing_text, date=date), warnings
