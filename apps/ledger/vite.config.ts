...
const { port } = loadSiteConfig(ROOT).ledger;

// SSR — no `prerender` block (Decision I): ledger runs as a TanStack Start server, a
...
export default defineConfig({
  ...
  // contentWatchPlugin runs build-content (→ src/generated/*.ts) at buildStart. For
  // ledger the "content" is the site registry joined to each target's public-origin
  // from config.kdl (no content files) — so re-runs are driven by config, watched at
  // buildStart. @tailwindcss/vite compiles gothic's theme.css (`@theme` tokens +
  // `@apply`); without it the gothic stylesheet ships raw and every var(--color-*)
  // is undefined.
  plugins: [
    contentWatchPlugin({
      root: ROOT,
