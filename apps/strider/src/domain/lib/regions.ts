...
export function foldRegions(layers: Layer[]): Region[] {
  ...
  return Array.from(state.values()).sort((a, b) => a.slug.localeCompare(b.slug));
}
...
export function foldFactionOverrides(layers: Layer[]): Map<string, string | null> {
...
export function foldSkein(layers: Layer[]): SkeinState {
    ...
    for (const change of layer.changes) {
      if (change.op === "skein-add") {
        ...
        if (change.from === change.to) {
          throw new Error(`layer ${layer.slug}: cannot skein-connect '${change.from}' to itself`);
        }
  ...
  const sortedRegions = Array.from(regions.values()).sort((a, b) => a.slug.localeCompare(b.slug));
