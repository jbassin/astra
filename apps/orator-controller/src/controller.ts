/**
 * Central controller: owns per-device navigation state, the visible-slot registry, the orator client,
...
import { back, type NavState, openCollection, rootNav, selectTag, withPage } from "./nav";
import { OratorClient, OratorError } from "./orator/client";
import type { Collection, NowPlaying, Tag, Track } from "./orator/types";
import { normalizeHex } from "./render/color";
import { messageSvg, type RenderOpts, renderRole } from "./render/svg";
...
function playbackErrorMessage(err: unknown): string {
  if (err instanceof OratorError && err.status === 409) return "Join a voice channel first";
  if (err instanceof OratorError && err.status === 503) return "orator bot offline";
  return "Playback failed";
}

export class OratorController {
  private client: OratorClient | null = null;
  private collections: Collection[] = [];
  private collectionsLoaded = false;
  private tagsLoaded = false;
  private now: NowPlaying | null = null;

  // Resolved fixed-tag taxonomy (rebuilt from orator's /tags).
  ...
  /** Bumped whenever the orator config changes, so stale in-flight loads don't write old data. */
  private generation = 0;

  // ---- configuration ----

  async init(): Promise<void> {
    const settings = (await streamDeck.settings.getGlobalSettings()) as OratorGlobalSettings;
    this.applySettings(settings);
    streamDeck.settings.onDidReceiveGlobalSettings((ev) => {
      this.applySettings(ev.settings as OratorGlobalSettings);
      void this.reloadAndRenderAll();
    });
  }

  private applySettings(settings: OratorGlobalSettings): void {
    this.generation++;
    // astra change (M4/slice 8): the orator-backend origin is configurable (birdfeed
    // hardcoded lark.iridi.cc). The operator sets both the origin + a pasted API key.
    this.client = isConfigured(settings)
      ? new OratorClient({ origin: normalizeOrigin(settings.oratorOrigin), key: settings.apiKey })
      : null;
  ...
  async onWillAppear(ev: WillAppearEvent): Promise<void> {
    ...
    if (!action.isKey()) return; // orator-controller is keypad-only
  ...
  async onKeyDown(ev: KeyDownEvent): Promise<void> {
    ...
    const role = roleAt(
      layout(ds.nav, ds.shape, this.gridData(ds)),
      { column: ref.column, row: ref.row },
      ds.shape,
    );
  ...
  private async toggleTrack(ref: SlotRef, trackId: number): Promise<void> {
    ...
    try {
      ...
      if (current === trackId && this.now?.status === "playing")
        this.now = await this.client.pause();
      else if (current === trackId && this.now?.status === "paused")
        this.now = await this.client.resume();
  ...
  /** Resolve the five named tag keys against orator tags (by lowercase name); "other" is the catch-all. */
  ...
  private deviceShape(action: KeyAction): DeviceShape {
    ...
    if (size?.columns && size.rows) return { columns: size.columns, rows: size.rows };
  ...
  private async renderSlot(context: string): Promise<void> {
    ...
    let image: string;
    if (!this.client) {
      image = messageSvg("Set orator origin + key in settings");
    } else {
      const role = roleAt(
        layout(ds.nav, ds.shape, this.gridData(ds)),
        { column: ref.column, row: ref.row },
  ...
  private async renderDevice(deviceId: string): Promise<void> {
    const contexts = [...this.slots.entries()]
