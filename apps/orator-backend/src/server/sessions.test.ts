...
describe("cookies", () => {
  test("sessionCookie sets HttpOnly + SameSite", () => {
    const c = sessionCookie("orator_session", "v");
    expect(c).toContain("HttpOnly");
    expect(c).toContain("SameSite=Lax");
    expect(c).toContain("Secure");
  });
  test("secure can be disabled for local http", () => {
    expect(sessionCookie("orator_session", "v", { secure: false })).not.toContain("Secure");
  });
  test("clearCookie expires immediately", () => {
    expect(clearCookie("orator_session")).toContain("Max-Age=0");
  });
