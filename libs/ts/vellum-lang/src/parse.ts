...
import { desugar } from "./surface";
import { parseTimeline } from "./timeline";
import { compileVss } from "./vss";

/** Container-directive names that drive layout/structure rather than naming a kind. */
...
/** Parse a *body* (frontmatter already stripped) into an mdast tree. Pure. Source
 * passes run first, all no-ops on input that doesn't use their syntax: `compileVss`
 * lowers the VSS structural surface (`@kind "…" { … }`, `@columns [ … ]`) to canonical
 * `:::` directives, `desugar` rewrites the inline sigils (`@action`, `||redact||`,
 * `#trait`), then remark parses; finally `transformCrossRefs` splits `[[…]]` runs out
 * of text nodes into `crossref` nodes. */
export function parseMarkdown(source: string): Root {
  const root = processor.parse(desugar(compileVss(source))) as Root;
  transformCrossRefs(root);
  return root;
}
...
function normalizeAttributes(attrs: ContainerDirective["attributes"]): Record<string, string> {
...
function inlineText(nodes: readonly PhrasingContent[]): string {
  ...
  for (const node of nodes) {
    if (node.type === "text" || node.type === "inlineCode") out += node.value;
      ...
      continue; // not readable label text
...
function parseNodes(content: readonly RootContent[]): VellumNode[] {
  ...
  for (const node of content) {
    ...
    } else if (node.type === "containerDirective" && node.name === COLUMNS_NAME) {
      flushProse();
      nodes.push(parseColumns(node));
    } else if (node.type === "containerDirective" && node.name === FIELDS_NAME) {
      flushProse();
      nodes.push(parseFields(node));
    } else if (node.type === "containerDirective" && node.name === TIMELINE_NAME) {
      flushProse();
      nodes.push(parseTimeline(node));
    } else {
      prose.push(node);
    }
  }
...
/**
 * Parse a document source into the vellum model. The leading YAML frontmatter is
 * split off + validated first; then top-level `:::kind` containers become blocks,
 * `:::columns`/`:::fields`/`:::timeline` become their structured nodes, and all other
 * top-level markdown (headings, lists, prose) is kept as prose runs in document order.
 * Mode defaults to mechanical; `opts.mode` overrides. Rules-illiterate — content is
 * carried verbatim and the parser never throws.
 */
export function parseDocument(source: string, opts?: { mode?: ThemeMode }): VellumDocument {
  const { yaml, body } = splitFrontmatter(source);
  const frontmatter = parseFrontmatter(yaml);
  const root = parseMarkdown(body);
  return {
    frontmatter,
    mode: opts?.mode ?? "mechanical",
    nodes: parseNodes(root.children),
  };
}
