      ...
      <aside className={styles.panel}>
        ...
        <div className={styles.modeRow}>
          {KIND_MODES[kind].map((m) => (
            <button
              key={m}
              type="button"
              className={state.mode === m ? styles.modeBtnActive : styles.modeBtn}
        ...
        {state.mode === "add" && (
          <section className={styles.section}>
            <div className={styles.hint}>Click hexes to toggle them into the new region.</div>
        ...
        {state.mode === "update" && (
          <section className={styles.section}>
            ...
            <div className={styles.readout}>
              ...
              <span>
                {pickedRegion
                  ? (factions.find((f) => f.slug === pickedRegion.faction)?.name ?? "—")
                  : "—"}
              </span>
            </div>
            <div className={styles.readout}>
        ...
        {state.mode === "skein-add" && (
          <section className={styles.section}>
            <div className={styles.hint}>Click a hex to place a new Skein node.</div>
            <label className={styles.field}>
              <span>Node name</span>
              <input
                type="text"
        ...
        {state.mode === "claim" && (
          <section className={styles.section}>
            <div className={styles.hint}>Click hexes to toggle them, then pick the new owner.</div>
            <label className={styles.field}>
              <span>New owner</span>
              <select
        ...
        <section className={styles.section}>
          <label className={styles.field}>
            ...
            <input
              ...
              onChange={(e) => dispatch({ type: "setLogMessage", value: e.target.value })}
          ...
          <label className={styles.field}>
            ...
            <input
              ...
              onChange={(e) => dispatch({ type: "setTimestamp", value: e.target.value })}
            />
          </label>
        </section>
          ...
          <div className={styles.preview}>
            {draftChange.op === "add" && (
              <>
                ...
                {draftChange.name}&rdquo; ({draftChange.faction}, {draftChange.hexes.length} hexes)
              </>
            )}
            {draftChange.op === "update" && (
              <>
                ...
                {[
                  ...
                  draftChange.hexes !== undefined && `${draftChange.hexes.length} hexes`,
                ]
                  .filter(Boolean)
                  .join(", ")}
                )
            ...
            {draftChange.op === "skein-add" && (
              <>
                ...
                {draftChange.name}&rdquo; ({draftChange.faction}, [{draftChange.hex[0]},{" "}
                {draftChange.hex[1]}])
              </>
            )}
            {draftChange.op === "skein-connect" && (
              <>
                ~ link <strong>{draftChange.from}</strong> ↔ <strong>{draftChange.to}</strong>
              </>
            )}
              ...
              <>
                ...
                <strong>
                  {draftChange.faction === null
                    ? "None"
                    : (factions.find((f) => f.slug === draftChange.faction)?.name ??
                      draftChange.faction)}
                </strong>
              </>
            )}
