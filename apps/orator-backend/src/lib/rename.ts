/**
 * Bulk-rename engine (lark B13). Pure + dependency-free so it is fully
 ...
 * before committing. Ported verbatim from faerrin `pkg/lark/src/lib/rename.ts`.
 */
  ...
  switch (op.kind) {
    ...
    case "replace": {
      ...
      while (i < title.length) {
        const hay = op.caseInsensitive
          ? title.slice(i, i + op.find.length).toLowerCase()
          : title.slice(i, i + op.find.length);
        if (hay === needle) {
          out += op.replaceWith;
          i += op.find.length;
        } else {
