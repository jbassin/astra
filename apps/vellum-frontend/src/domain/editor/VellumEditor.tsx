...
export function VellumEditor() {
  ...
  // Debounced persist of the active doc's source (R-19 autosave). Keyed on
  // `source`; the store is read inside the setter, so no extra deps are needed.
  useEffect(() => {
    const id = setTimeout(() => {
      setStore((prev) => {
  ...
  const openDoc = useCallback((next: DocStore) => {
    saveStore(next);
    setStore(next);
  ...
  const flushed = useCallback(() => updateActiveSource(store, source), [store, source]);
    ...
    <div className={styles.app}>
      <header className={styles.toolbar}>
        ...
        <div className={styles.docbar}>
          ...
          <button type="button" className={styles.iconButton} onClick={newDoc} title="New document">
            ＋
          </button>
          <button
            type="button"
