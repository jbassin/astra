...
/** In-memory PlaybackStore (the engine only needs getTrack + markTrackError). */
class FakePlaybackStore implements PlaybackStore {
  ...
  add(
    title: string,
    filePath: string | null = `/data/${title}.ogg`,
    lufs: number | null = -20,
  ): number {
    const id = ++this.seq;
    this.tracks.set(id, {
...
function mkTrack(
  ...
  return store.add(title, filePath, lufs);
}
...
function engine(opts?: {
  ...
  return new PlaybackEngine({ store, voice, resolver, targetLufs: -16, ...opts });
}

beforeEach(() => {
  store = new FakePlaybackStore();
  voice = new FakeVoice();
});
...
describe("resilient skip (B10)", () => {
  test("a track that fails to play is skipped and marked error", async () => {
    ...
    await e.play({ trackIds: [a, b], userId: "op" });
    expect(voice.playing).toBe("/data/b.ogg");
    expect((await store.getTrack(a))?.status).toBe("error");
  });
...
describe("auto-leave (B2)", () => {
  test("empty channel arms a timer that leaves; rejoin cancels it", async () => {
    // Holder (not a bare `let`) so TS keeps the callback's type instead of
    // narrowing to the initial `null` (the assignment happens inside setTimer).
    const armed: { fn: (() => void) | null } = { fn: null };
    const setTimer = ((f: () => void) => {
      armed.fn = f;
      return 1 as unknown as ReturnType<typeof setTimeout>;
    }) as typeof setTimeout;
    const cleared: number[] = [];
    const clearTimer = ((t: number) => cleared.push(t)) as unknown as typeof clearTimeout;
    const e = engine({ autoLeaveMs: 60_000, setTimer, clearTimer });
    await e.play({ trackIds: [mkTrack("a")], userId: "op" });

    e.notifyPopulation(0); // everyone left → arm
    expect(typeof armed.fn).toBe("function");
...
describe("async channel resolution (REST fallback, D8)", () => {
  test("play awaits an async resolver and joins the resolved channel", async () => {
    ...
    const e = new PlaybackEngine({ store, voice, resolver: asyncResolver, targetLufs: -16 });
    await e.play({ trackIds: [mkTrack("a")], userId: "op" });
  ...
  test("409 when the async resolver finds no channel", async () => {
    ...
    const e = new PlaybackEngine({ store, voice, resolver: asyncResolver, targetLufs: -16 });
    await expect(e.play({ trackIds: [mkTrack("a")], userId: "ghost" })).rejects.toMatchObject({
      status: 409,
    });
  });
});
