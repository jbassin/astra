...
import { openCollection, rootNav, selectTag, withPage } from "./nav";
import type { Collection, Tag, Track } from "./orator/types";
...
function tagData(
  ...
  resolved: TagKey[] = ["explore", "stealth", "battle", "calm", "dungeon", "other"],
): GridData {
...
describe("layout — root", () => {
  ...
  test("(0,0) reserved; collections start at key 1", () => {
    ...
    expect(roleAt(grid, { column: 1, row: 0 }, XL)).toEqual({
      kind: "collection",
      id: 1,
      name: "A",
    });
    expect(roleAt(grid, { column: 2, row: 0 }, XL)).toEqual({
      kind: "collection",
      id: 2,
      name: "B",
    });
...
describe("layout — tag page (XL 8×4)", () => {
  ...
  test("rightmost column = Back, explore, stealth, other", () => {
    ...
    expect(roleAt(grid, { column: 7, row: 1 }, XL)).toMatchObject({
      kind: "navTag",
      key: "explore",
      label: "Explore",
    });
    expect(roleAt(grid, { column: 7, row: 2 }, XL)).toMatchObject({
      kind: "navTag",
      key: "stealth",
    });
  ...
  test("next column = play/pause, battle, calm, dungeon (calm active by default)", () => {
    ...
    expect(roleAt(grid, { column: 6, row: 1 }, XL)).toMatchObject({
      kind: "navTag",
      key: "battle",
      active: false,
    });
    expect(roleAt(grid, { column: 6, row: 2 }, XL)).toMatchObject({
      kind: "navTag",
      key: "calm",
      active: true,
    });
    expect(roleAt(grid, { column: 6, row: 3 }, XL)).toMatchObject({
      kind: "navTag",
      key: "dungeon",
    });
  });
  ...
  test("page-control column = info, next?, prev?, stop", () => {
    expect(roleAt(grid, { column: 5, row: 0 }, XL)).toEqual({
      kind: "pageInfo",
      page: 1,
      total: 1,
    });
...
describe("layout — track tiling (column-major, skips (0,0), colored)", () => {
  ...
  test("tracks tile top→bottom then left→right, starting below the reserved corner", () => {
    ...
    expect(roleAt(grid, { column: 0, row: 1 }, XL)).toMatchObject({
      kind: "track",
      id: 100,
      color: "#4aa6a0",
    });
