/**
 * Multi-document store (R-19). Pure reducers over a {docs, activeId} state plus
 * thin localStorage load/save with migration from the old single-document key.
 * The editor is one active doc at a time; new/switch/rename/delete operate here.
 * Ported verbatim from faerrin pkg/vellum.
 */
...
export function createDoc(source = "", opts?: { id?: string; now?: number }): VellumDoc {
  return {
    id: opts?.id ?? newId(),
...
export function emptyStore(source = "", opts?: { id?: string; now?: number }): DocStore {
...
export function renameActive(store: DocStore, title: string): DocStore {
  ...
  return {
    ...store,
    docs: store.docs.map((d) =>
      d.id === store.activeId ? { ...d, title: clean || d.title, titlePinned: true } : d,
    ),
  };
}
...
export function deleteDoc(store: DocStore, id: string): DocStore {
  ...
  const activeId = store.activeId === id ? docs[docs.length - 1]!.id : store.activeId;
