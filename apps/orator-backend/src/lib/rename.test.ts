...
describe("applyRename", () => {
  test("literal replace-all", () => {
    expect(
      applyRename("OST - Track (Official)", [
        { kind: "replace", find: " (Official)", replaceWith: "" },
      ]),
    ).toBe("OST - Track");
  });
  test("case-insensitive literal replace", () => {
    expect(
      applyRename("HELLO hello", [
        { kind: "replace", find: "hello", replaceWith: "x", caseInsensitive: true },
      ]),
    ).toBe("x x");
  });
  test("regex replace with capture", () => {
    expect(
      applyRename("12 - Boss Theme", [
  ...
  test("invalid regex throws RenameError", () => {
    expect(() =>
      applyRename("x", [{ kind: "replace", find: "(", replaceWith: "", regex: true }]),
    ).toThrow(RenameError);
  });
});
...
describe("previewBulkRename", () => {
  test("marks changed rows and never mutates input", () => {
    ...
    const rows = previewBulkRename(items, [
      { kind: "replace", find: "^\\d+ - ", replaceWith: "", regex: true },
    ]);
    expect(rows).toEqual([
