 ...
 * moves into the modal on open and is restored to the trigger on close. Ported
 * from faerrin lark `src/web/ui/Dialog.tsx`.
 */
import {
  createContext,
      ...
      >
        ...
        {kind === "prompt" ? (
          <div className="modal__body">
            {(opts as PromptOpts).label && (
              <span className="modal__label">{(opts as PromptOpts).label}</span>
            )}
            <input
          ...
          (opts as ConfirmOpts).body && (
            <div className="modal__body">{(opts as ConfirmOpts).body}</div>
          )
        )}
