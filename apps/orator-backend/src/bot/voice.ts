/**
 * Voice abstraction (lifted from faerrin lark). The playback engine talks only to
 * this interface, so its queue/loop/gain/auto-leave logic is unit-tested with a
 * fake, while the real implementation (`discord-voice.ts`) wraps @discordjs/voice
 * in-process.
 *
