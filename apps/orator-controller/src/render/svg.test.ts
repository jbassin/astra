...
describe("svg rendering", () => {
  ...
  test("play/pause key shows Pause when playing, Play otherwise", () => {
    expect(decodeURIComponent(renderRole({ kind: "playPause" }, { playing: true }))).toContain(
      "Pause",
    );
    expect(decodeURIComponent(renderRole({ kind: "playPause" }, { playing: false }))).toContain(
      "Play",
    );
  });
  ...
  test("track tile background = its tag color", () => {
    const svg = decodeURIComponent(
      renderRole({ kind: "track", id: 1, title: "X", color: "#c8504a" }),
    );
  ...
  test("a playing track draws a highlight border + progress", () => {
    const svg = decodeURIComponent(
      renderRole(
        { kind: "track", id: 1, title: "X", color: "#4aa6a0" },
        { playing: true, positionMs: 5, durationMs: 10 },
      ),
    );
