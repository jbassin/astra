...
export interface OratorConfig {
  /** e.g. "https://orator.iridi.cc" or "http://localhost:8788". Trailing slash tolerated. */
  origin: string;
  /** A `orator_…` API key minted in orator's web UI. Sent as `Authorization: Bearer …`. */
  key: string;
}

/** Thrown for any non-2xx orator response. `status` carries the HTTP code (401/409/503/…). */
 ...
 * Only defined filters are emitted, so the orator default (limit 200) applies when omitted.
 */
...
/** Thin REST client for orator's Stream Deck API. Uses Node 20's global `fetch`. */
export class OratorClient {
  private readonly origin: string;
  private readonly key: string;

  constructor(config: OratorConfig) {
