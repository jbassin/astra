import { type EditableChange, layerFilename, serializeLayer, slugify } from "@/lib/editorHelpers";
...
export async function saveLayer({ draftChange, logMessage, timestamp }: SaveArgs): Promise<void> {
  ...
  try {
    ...
    throw new Error(`${msg} — is the sidecar running? \`bun run editor:server\``);
  }
