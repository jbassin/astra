/**
 * The heart of orator-controller: given the navigation level, the device grid shape, and resolved library
...
import type { Collection, Track } from "./orator/types";
import { type TagKey, tagLabel } from "./tags";
...
function fillRegion(cells: Role[], indices: number[], items: Role[], page: number): void {
  ...
  if (n === 0) return;
  const put = (cellIdx: number | undefined, role: Role) => {
    if (cellIdx !== undefined) cells[cellIdx] = role;
  };
  if (items.length <= n) {
    items.forEach((it, i) => {
      put(indices[i], it);
  ...
  put(indices[n - 2], p > 0 ? { kind: "pagePrev" } : EMPTY);
  put(indices[n - 1], p < pageCount - 1 ? { kind: "pageNext" } : EMPTY);
}
...
/** Tracks matching the active selector. namedTagIds maps the 5 named keys to a orator tag id. */
export function tracksForSelector(
  tracks: Track[],
  key: TagKey,
  namedTagIds: Map<TagKey, number>,
): Track[] {
  if (key === "other") {
...
export function layout(nav: NavState, device: DeviceShape, data: GridData): Role[] {
  ...
  slice.forEach((t, i) => {
    const idx = cellIdx[i];
    if (idx !== undefined)
      cells[idx] = { kind: "track", id: t.id, title: t.title, color: data.activeColor };
  });
