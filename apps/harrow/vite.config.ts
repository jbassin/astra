...
const { port } = loadSiteConfig(ROOT).harrow;

// SSR — no `prerender` block (Decision I): harrow runs as a TanStack Start server, a
...
export default defineConfig({
  ...
  // re-runs on content edits in dev, so the .card/.spread parsing never reaches the
  ...
  plugins: [
    contentWatchPlugin({
      ...
      invalidate: ["site.ts", "cards.ts", "spreads.ts"],
    }),
