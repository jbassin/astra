/**
 * Discord bot bootstrap (lark §4/B1/B2) — all in-process under Bun. Builds the
 * client, the follow-the-operator resolver, the in-process voice adapter, and the
 * playback engine, and wires voice-state changes to the 60s auto-leave. Started
 * from the entrypoint only when a token is configured. Not unit-tested.
 *
...
export async function startBot(opts: {
  ...
  const resolver: VoiceStateResolver = {
    async channelOf(userId) {
      ...
      const cached =
        client.guilds.cache.get(opts.guildId)?.voiceStates.cache.get(userId)?.channelId ?? null;
      ...
      try {
        ...
        if ((err as { status?: number }).status !== 404) {
          console.error(
            `[orator] voice-state lookup failed for ${userId}:`,
  ...
  console.log(`[orator] bot ready as ${client.user?.tag} in "${guild?.name ?? opts.guildId}"`);
