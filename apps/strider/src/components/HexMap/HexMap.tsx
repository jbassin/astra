...
import { computeRegionBorders, type EdgeSegment, hexPixel } from "@/lib/hexUtils";
import type { LayerAnimation, Region, SkeinRegion, SkeinState } from "@/lib/layers";
...
export default function HexMap(props: HexMapProps) {
  ...
  useEffect(() => {
    ...
    const fitToHost = () => {
      ...
      const scale = Math.min(rect.width / WORLD_VIEWBOX.width, rect.height / WORLD_VIEWBOX.height);
      world.scale.set(scale);
      world.position.set(rect.left + rect.width / 2, rect.top + rect.height / 2);
  ...
  useEffect(() => {
    sceneRef.current?.setFactionState(props.factionHexes, props.unownedHexes, props.factionBorders);
...
function buildScene(
  ...
  function buildPulseEntry(curve: SkeinCurve, sig: SkeinSignature): PulseEntry | null {
  ...
  async function setSkein(skein: SkeinState) {
    ...
    for (const [key, ep] of desired) {
      ...
      const endpointsChanged =
        !!cg && (cg.x1 !== ep.x1 || cg.y1 !== ep.y1 || cg.x2 !== ep.x2 || cg.y2 !== ep.y2);
      if (!cg) {
  ...
  function animateSkeinLink(from: string, to: string, durationMs: number): void {
  ...
  function animateFactionFlips(
    ...
    const entries: FlipEntry[] = ordered.map(({ hex, d }) => {
      ...
      const prevColor =
        prevIdx === null ? "#787c80" : (propsRef.current.factions[prevIdx]?.color ?? "#787c80");
    ...
    const totalMs = maxRing * waveStepMs + PER_HEX_FLIP_MS + 20; /* small tail */
    animMgr.start({
      startAt,
      durationMs: totalMs,
      update: () => {
